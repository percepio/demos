...
int dfm_test_run_t17(const char *test_id)
{
	...
	DFM_TRAP(1017, "Test 1017: Expected: trap.zpr only; trace disabled", 0);
...
{
	ARG_UNUSED(test_id);
	DFM_TRAP(1020, "Test 1020A: Expected: no trap.zpr; coredump too small", 0);
	(void)xTracePrint(dfm_test_trace_channel(),
		"T1020B RETURN first trap completed");
	DFM_TRAP(1020, "Test 1020B: Expected: no trap.zpr; trace proves return", 0);
	return 0;
}

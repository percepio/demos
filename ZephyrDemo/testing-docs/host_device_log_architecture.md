...
named session log. The Python runner uses pyserial at 115200 baud, opens the
selected port before every flash, mirrors all decoded target output into the
console and `dfm_test_run.log`, and preserves exact bytes in each artifact
...
first 1024 received bytes. The selected image is flashed again for the
authoritative run capture. An unsuccessful full pass produces a warning and
the search repeats. The selected port is retained for every variant; a later
open/read failure fails the affected run instead of changing devices silently.
...
description-length, and target-failure checks remain in place. The runner now
...
5. During discovery, build the first image, open each candidate serial port,
   then flash while its reader is active. Reflash after selection for the
   authoritative run and retain raw bytes in a stable per-image artifact path.

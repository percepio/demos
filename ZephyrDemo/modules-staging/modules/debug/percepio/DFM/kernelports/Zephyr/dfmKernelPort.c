...
/*
	...
	#if !(defined(CONFIG_PERCEPIO_DFM_CFG_RETAINED_MEMORY) && CONFIG_PERCEPIO_DFM_CFG_RETAINED_MEMORY == 1)
...
#if (defined(CONFIG_PERCEPIO_DFM_CFG_COREDUMP_RETAIN) && CONFIG_PERCEPIO_DFM_CFG_COREDUMP_RETAIN == 1) && \
   !(defined(CONFIG_PERCEPIO_DFM_CFG_RETAINED_MEMORY) && CONFIG_PERCEPIO_DFM_CFG_RETAINED_MEMORY == 1)
#error "DFM is configured to store Core Dumps in Retained Memory but that isn't enabled in DFM."
#endif
...
const char *zephyr_reason_to_str(unsigned int reason)
{
    ...
    case K_ERR_STACK_CHK_FAIL:  return "Stack check fail";
    case K_ERR_SPURIOUS_IRQ:    return "Spurious IRQ";    
    default:                    return "Other fault";
    }
}
...
static void xDfmCoredumpBackendEnd(void)
{
	...
	const char* reasonStr = zephyr_reason_to_str(pxCoredumpHeader->reason);	
	DFM_CFG_PRINT(LNBR "Fault reason: ");
	DFM_CFG_PRINT(reasonStr);
	DFM_CFG_PRINT(LNBR);

	if (xDfmAlertBegin(DFM_TYPE_HARDFAULT, reasonStr, &xAlertHandle) == DFM_SUCCESS)
	{

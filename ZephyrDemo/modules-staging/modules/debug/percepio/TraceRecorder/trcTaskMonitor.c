...
volatile int ignoreDirtyPeriod = 0;
...
traceResult xTraceTaskMonitorPrint(void)
{
	...
	for (i = 0; i < TRC_CFG_TASK_MONITOR_MAX_TASKS; i++)
	{
		...
		TRC_CFG_PRINTF("%-5lu%-20s%-8lu%-8lu\n", (long unsigned int)i, szName, (long unsigned int)pxTaskData.uxWatermarkLow, (long unsigned int)pxTaskData.uxWatermarkHigh);
	}
	TRC_CFG_PRINTF("\n");

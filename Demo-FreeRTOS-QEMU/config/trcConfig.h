 ...
 * the generic FreeRTOS fallback uses taskENTER_CRITICAL(), which is deliberately
 * rejected by the Cortex-M3 port when called from an exception. */
#define TRC_CFG_ALLOC_CRITICAL_SECTION() \
    TraceUnsignedBaseType_t TRACE_ALLOC_CRITICAL_SECTION_NAME
#define TRC_CFG_ENTER_CRITICAL_SECTION() \
    do { TRACE_ALLOC_CRITICAL_SECTION_NAME = __get_PRIMASK(); __set_PRIMASK(1U); } while (0)
#define TRC_CFG_EXIT_CRITICAL_SECTION() \
    do { __set_PRIMASK(TRACE_ALLOC_CRITICAL_SECTION_NAME); } while (0)

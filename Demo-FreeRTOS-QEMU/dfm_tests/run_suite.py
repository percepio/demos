#!/usr/bin/env python3
"""Build and run the sequential FreeRTOS DFM test variants.
...
# reviewed test harness, not a general FreeRTOS build frontend.
...
def report_test_case_results(
    ...
    for test_id in test_ids:
        ...
        alert_type = int(test_id)
        expected_alerts = TESTCASE_ALERT_COUNTS[test_id]
        actual_alerts = actual_alert_counts[alert_type]
        alert_count_ok = actual_alerts == expected_alerts
        descriptions_ok = alert_type not in too_long_description_types
...
def stop_process_tree(
    ...
    # A caller may provide a QEMU pidfile. It is removed before launch, so this
    # PID cannot be stale from an earlier suite invocation.
...
def configured_zephyr_sdk() -> Path | None:
    """Return an explicitly configured Zephyr SDK, if present."""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"FreeRTOS target name; default: {DEFAULT_BOARD}",
    )
...
def _run_suite(args: argparse.Namespace, state: SuiteRunState) -> int:
    ...
    with LOG_PATH.open("w", encoding="utf-8", newline="\n") as log_file:
        ...
        if cmake is None:
            reporter.failed("Could not find CMake on PATH")
            return 2
        sdk_path = configured_zephyr_sdk()
        environment = child_environment(sdk_path)
        reporter.info(f"CMake command: {cmake}")
        reporter.info(
            "Arm toolchain SDK: "
        ...
        for variant in variants:
            ...
            if configure.return_code != 0 or configure.timed_out:
                reporter.failed(f"Configure {variant.name}")
                report_unrun_test_cases(
                    selected_test_ids, reporter, "firmware configure failed"
                )
            ...
            try:
                ...
                reporter.failed(
                    f"Could not verify build configuration {config_path}: {error}"
                )
                report_unrun_test_cases(
                    selected_test_ids,
                    reporter,
                    "firmware build configuration could not be verified",
                )
                failed_variants.append(variant.name)
                continue
            if (
                ...
                reporter.failed(
                    f"Build {variant.name} has unexpected configuration; "
                    f"expected FreeRTOS/{variant.name}/{variant.revision}/"
                    f"test {testcase or 0}"
                )
                report_unrun_test_cases(
                    selected_test_ids,
                    reporter,
                    "firmware build metadata is incorrect",
                )
                failed_variants.append(variant.name)
                continue
            reporter.passed(
            ...
            reporter.info(f"Build map: {map_path}")
            reporter.info(f"Build metadata: {config_path}")
            reporter.info(f"Saved ELF: {artifact_elf}")
            reporter.info(f"Saved map: {artifact_map}")

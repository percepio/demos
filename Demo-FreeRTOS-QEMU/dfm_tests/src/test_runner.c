...
static StaticTask_t test_runner_tcb;
static StackType_t test_runner_stack[DFM_TEST_RUNNER_STACK_WORDS];
...
#endif
	/* Keep the pre-initialization robustness test last so a regression cannot
	 * hide evidence from the ordinary -Os cases.
	 */
...
{
	uint32_t run_id = (uint32_t)DFM_TEST_RUN_COOKIE;
...
static void prepare_trace_recorder(void)
{
	...
	if (!xTraceIsRecorderEnabled()) {
		...
		dfm_test_check("HARNESS", result == TRC_SUCCESS, "TRACE_STARTED");
	}
...
static void advance_after_interrupted_case(struct dfm_test_state_snapshot *state)
{
	...
	printk("DFMT:HARNESS_FAIL:%s:INTERRUPTED:phase=%u:reason=%u\n",
		test_id, (unsigned int)previous_phase,
		(unsigned int)state->observation_a);
...
static void recover_previous_boot(struct dfm_test_state_snapshot *state)
{
	...
	case DFM_TEST_PHASE_STARTUP_DONE:
		if ((state->next_index < DFM_TEST_CASE_COUNT) &&
		    (test_registry[state->next_index].numeric_id ==
		     state->armed_test)) {
			const struct dfm_test_case *test =
				&test_registry[state->next_index];
			uint32_t completed_id = state->armed_test;

			printk("DFMT:RETURNED:%s:startup:ipsr=%u:control=0x%x\n",
				test->id, (unsigned int)state->observation_a,
				(unsigned int)state->observation_b);
	...
	const struct dfm_test_case *test)
{
	...
	for (drain = 0U; drain < 10000U; drain++) {
		__asm volatile("nop");
	}
	NVIC_SystemReset();
	for (;;) { }
...
static void dfm_test_runner_task(void *parameter)
{
	...
	if (!dfm_test_state_load(&state)) {
		...
		printk("DFMT:BOOT:%s:%08x:phase=%u:index=%u:reboots=%u\n",
			DFM_TEST_VARIANT_NAME, (unsigned int)state.run_id,
			(unsigned int)state.phase, (unsigned int)state.next_index,
			(unsigned int)state.reboot_count);
	}
	...
	while (state.next_index < DFM_TEST_CASE_COUNT) {
		...
		if ((test->kind == DFM_TEST_KIND_DFM_REBOOT) ||
		    (test->kind == DFM_TEST_KIND_FAULT_REBOOT)) {
			const char *reason = test->kind == DFM_TEST_KIND_DFM_REBOOT ?
				"dfm" : "fault";
			...
			state.reboot_count++;
			dfm_test_state_commit(&state);
			printk("DFMT:REBOOT_EXPECTED:%s:%s\n", test->id, reason);
			result = test->function(test->id);
			printk("DFMT:HARNESS_FAIL:%s:EXPECTED_RESTART_RETURNED:%d\n",
				test->id, result);
		} else {
...
int run_tests(void)
{
	...
	printk("Booting Percepio FreeRTOS DFM test harness on QEMU mps2-an385\n");
	dfm_test_run_pre_init_startup();

	if (xTraceInitialize() != TRC_SUCCESS) {
		printk("DFMT:HARNESS_FAIL:BOOT:TRACE_INITIALIZE\n");
		return -1;
	}
	if (xDfmInitializeForLocalUse() != DFM_SUCCESS) {
		printk("DFMT:HARNESS_FAIL:BOOT:DFM_INITIALIZE\n");
		return -1;
	}
	if (xTraceEnable(TRC_START) != TRC_SUCCESS) {
		printk("DFMT:HARNESS_FAIL:BOOT:TRACE_START\n");
		return -1;
	}

	dfm_test_run_post_init_startup();
	task = xTaskCreateStatic(dfm_test_runner_task, "DFMTests",
		DFM_TEST_RUNNER_STACK_WORDS, NULL, DFM_TEST_RUNNER_PRIORITY,
		test_runner_stack, &test_runner_tcb);
	if (task == NULL) {
		printk("DFMT:HARNESS_FAIL:BOOT:TASK_CREATE\n");
		return -1;
	}

	vTaskStartScheduler();
	printk("DFMT:HARNESS_FAIL:BOOT:SCHEDULER_RETURNED\n");
	return -1;
}
